...
export class MongoUserRepository implements UserRepository {
  ...
  async countSuperAdmins(): Promise<number> {
    return this.userModel.countDocuments({ role: UserRole.SUPER_ADMIN });
  }
  ...
  async updateBatch(users: DomainUser[]): Promise<DomainUser[]> {
    const operations = users.map((user) => ({
      updateOne: {
        filter: { _id: user.id },
        update: this.domainToMongo(user),
      },
    }));
  ...
  async save(user: DomainUser): Promise<DomainUser> {
    try {
      ...
      return this.mongoToDomain(savedUser);
    } catch (error) {
      this.logger.error(
  ...
  async findWithPagination(
    ...
    try {
      ...
      const data = users.map((user: UserDocument) => this.mongoToDomain(user));
   ...
   */
  private domainToMongo(user: DomainUser): Partial<UserDocument> {
    return {
      _id: user.id,
      email: user.email.value,
      name: user.name,
   ...
   */
  private mongoToDomain(userDoc: UserDocument): DomainUser {
    return DomainUser.createWithHashedPassword(
      userDoc._id,
