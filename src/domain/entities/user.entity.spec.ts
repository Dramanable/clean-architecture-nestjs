...
describe('User Entity with Roles', () => {
  ...
  describe('Permissions System', () => {
    it('should allow super admin to have all permissions', () => {
      // Arrange
      const superAdmin = new User(validEmail, 'Super Admin', UserRole.SUPER_ADMIN);
  ...
  describe('Business Rules - User Actions', () => {
    it('should allow super admin to act on any user', () => {
      const superAdmin = new User(validEmail, 'Super Admin', UserRole.SUPER_ADMIN);
      const regularUser = new User(new Email('user@example.com'), 'User', UserRole.USER);
      const manager = new User(new Email('manager@example.com'), 'Manager', UserRole.MANAGER);
    ...
    it('should allow manager to act on regular users only', () => {
      const manager = new User(validEmail, 'Manager', UserRole.MANAGER);
      const regularUser = new User(new Email('user@example.com'), 'User', UserRole.USER);
      const anotherManager = new User(new Email('manager2@example.com'), 'Manager 2', UserRole.MANAGER);
    ...
    it('should allow regular user to act only on themselves', () => {
      ...
      const anotherUser = new User(new Email('other@example.com'), 'Other User', UserRole.USER);
  ...
  describe('Role Validation', () => {
    it('should reject invalid name', () => {
      expect(() => new User(validEmail, '', UserRole.USER))
