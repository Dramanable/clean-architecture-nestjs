...
export class GetUserUseCase {
  ...
  private validateViewPermissions(
    ...
    // 2. Les utilisateurs réguliers ne peuvent voir que leur propre profil
    ...
    // 3. Vérification des permissions VIEW_USER pour managers et admins
    if (!requestingUser.hasPermission(Permission.VIEW_USER)) {
      this.logger.warn(this.i18n.t('warnings.permission.denied'), {
        requestingUserId: requestingUser.id,
        requestingUserRole: requestingUser.role,
        requiredPermission: Permission.VIEW_USER,
    ...
    // 4. Les managers ne peuvent voir que les utilisateurs réguliers
